...
async function main() {
    ...
    result = await resetVSSGroupPromise(chain3);
    console.log("reset vss group transaction with hash: " + result);
}

function resetVSSGroupPromise(chain3) {
    return new Promise((resolve, reject) => {
        resetVSSGroupTransaction = {
            from: install_account,
		    to: vssbase.address,
            gas: "1000000",
		    data: vssbase.resetVSSGroup.getData()
        };
