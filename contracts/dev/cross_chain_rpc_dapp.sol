...
contract Precompiled11 {
  function crossChainRPC (address, bytes4, bytes) public;
}

contract crossChainRPC is DappBase {
  Precompiled11 constant PREC11 = Precompiled11(0xB);

  function crossChainRPC() public payable {
  }

  // callRPC("0x0123456789abcdef", "save(string,uint256,bool)", "hello world", 3, true)
  function callRPC(address subchainAddr, string funcSignature, string arg1, uint256 arg2, bool arg3) public {
    bytes4 funcSignatureEncoded = bytes4(keccak256(bytes(funcSignature)));
    bytes memory abiEncoded = abi.encode(arg1, arg2, arg3);
    PREC11.crossChainRPC(subchainAddr, funcSignatureEncoded, abiEncoded);
  }
}
